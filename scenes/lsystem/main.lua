...
function Turtle:rotateRight()
    self.dir:rotateInplace(math.pi / 2)
end
...
function rewrite(axiom, rulesTable, iterCount)
    ...
    for i = 1, iterCount do
        ...
        print("iteration", i, "str", str)
    end
    return str
end

function Turtle:prepare(axiom, rules, iterations)
    local result
    local ok, msg = pcall(function()
        ...
        print("axiom", axiom)
        result = rewrite(axiom, rulesTable, iterations)
        --print("str", str)
    end)
    print(ok, msg)
    return result
...
function Turtle:start(code, iterations)
    ...
    local code = self:prepare(axiom, rules, iterations)
...
function resetAndRunAxiom()
    turtle = Turtle:new(width() / 2, height() / 2, - 1 / 2 * math.pi)
    turtle.executor = coroutine.create(turtle.body)
    coroutine.resume(turtle.executor, turtle, axiom)
    clearCanvas()
end
...
function setupWindow()
    ...
    if ui.Button("run axiom only") then
        resetAndRunAxiom()
    end
    ui.End()
end
...
function rulesWindow()
    ui.Begin("rules", false, { "ImGuiWindowFlags_AlwaysAutoResize" })
